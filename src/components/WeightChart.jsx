...
function ChartTooltip({ active, payload, label }) {
  ...
  return (
    <div className="rounded-md border border-white/80 bg-stone-900 px-3 py-2 text-xs text-white shadow-soft">
      ...
      <strong className="ml-2">{payload[0].value.toFixed(1)} kg</strong>
    </div>
  );
}
...
export function WeightChart({ entries, goalWeight }) {
  ...
  const data = entries.map((entry) => ({ label: formatEntryDate(entry.date), weight: entry.weight }));
    ...
    <div className="h-[250px] w-full lg:h-[315px]">
      <ResponsiveContainer width="100%" height="100%">
        <ComposedChart data={data} margin={{ top: 14, right: 10, left: 4, bottom: 0 }}>
          ...
          {goalWeight >= min && goalWeight <= max && <ReferenceLine y={goalWeight} stroke="#5b8f7d" strokeDasharray="5 5" strokeOpacity={0.5} />}
